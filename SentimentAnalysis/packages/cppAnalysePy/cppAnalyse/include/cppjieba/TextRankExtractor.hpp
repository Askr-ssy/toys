...
public:
    ...
    {
        string word;
        vector<size_t> offsets; //存在大小 ?vector?
        double weight;
    } Word; // struct Word
private:
    typedef std::map<string, Word> WordMap;     //词-对象权重
    ...
    private:
        ...
        typedef std::map<Node, double> Edges;   //对方词-权重
        typedef std::map<Node, Edges> Graph;    //节点<对方节点-权重>
    ...
    public:
        ...
        void rank(WordMap &ws, size_t rankTime = 10)
        {
            ...
            for (size_t i = 0; i < rankTime; i++)//重新分配权重
            {
